...
export default function ProjectMap() {
  ...
  const { data: mapData, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get(`/geospatial/projects/${projectId}/map-features`)
      ...
      <div className="h-full flex flex-col">
        ...
        <div className="bg-white border-b px-6 py-4 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Mapa del Proyecto</h1>
              {mapData && <p className="text-gray-600 text-sm mt-1">{mapData.project_name}</p>}
            </div>
        ...
        <div className="flex flex-1 gap-4 p-4 bg-gray-50">
          {/* Map */}
          <div className="flex-1 rounded-lg shadow-md overflow-hidden bg-white">
            {isLoading ? (
              ...
              <div className="bg-white rounded-lg shadow-md p-4">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <p className="text-gray-600">Puntos topográficos:</p>
                    <p className="font-semibold text-gray-900">{mapData.survey_points.length}</p>
                  </div>
                  <div className="flex justify-between">
                    <p className="text-gray-600">Vuelos con dron:</p>
                    <p className="font-semibold text-gray-900">{mapData.drone_coverage.length}</p>
                  </div>
                  <div className="flex justify-between">
                    <p className="text-gray-600">Parcelas:</p>
                    <p className="font-semibold text-gray-900">{mapData.parcels.length}</p>
                  </div>
                  {mapData.drone_coverage.length > 0 && (
                    <div className="flex justify-between pt-2 border-t">
                      <p className="text-gray-600">Cobertura total:</p>
                      <p className="font-semibold text-gray-900">
                        {mapData.drone_coverage.reduce((sum: number, d: any) => sum + (d.area_coverage_hectares || 0), 0).toFixed(2)} ha
                      </p>
                    </div>
                  )}
                </div>
